...
class GeneralMetricsAnalyzer(JsonMetricsAnalyzer):
    ...
    def process(self, metric_info: dict) -> None:
        if "cells" not in metric_info:
            return  # Filtering other metrics
        ...
        for cell_info in metric_info["cells"]:
            ...
            if "ue_list" in cell_info:
                if cell_info["ue_list"] and self._time_first is None:
                    # Once the first metric report with ues is received, set timers for bitrate calculation
                    # If we had a previous report, that's the first timestamp
                    if self._time_last is None:
                        self._time_last = timestamp
                    self._time_first = self._time_last
    ...
    def _update_bitrate(self, timestamp: datetime.datetime, dl_brate: float, ul_brate: float) -> None:
        if self._time_first is None or self._time_last is None:
            return  # We only process the bitrate after the first metric report with ues

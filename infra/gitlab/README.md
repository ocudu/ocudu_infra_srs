...
This Terraform module manages the complete configuration of this GitLab project, including project settings, pipeline schedules, protected branches, approval rules and more.

## Using it in Gitlab CI

This module is designed to work with the GitLab CI Terraform integration. When the terraform module changes, the CI pipeline automatically:
...
- Update the required values in `terraform.tfvars` and commit them (MR or main branch)

## Alternative: Manual Usage

1. Configure the required variables in a new `terraform.tfvars` file.
2. Set up the required GitLab token (see Security Requirements below)
3. Set up environmental variables `TF_VAR_CI_PROJECT_ID`, `TF_VAR_CI_PROJECT_NAME` and `GITLAB_BASE_URL` (in case not using gitlab.com)
4. Run Terraform:
